...
function query(queryName, term, confirmationSearch=false) {
  if(!confirmationSearch) {
    return axios.post(url, queryName(term), config);
  }
  ...
  return axios.get('https://api.github.com/rate_limit?' +
    ...
    .then( (res) => {
        ...
        if (remainingReq < 1) return axios.post(url, queryName(term), config);
        ...
        .then( (res) => {
          ...
          return axios.post(url, queryName(newTerm), config);;
        });
    });
}
