...
function renderPills(languages) {
  return languages.map((language) => {
    return (
      <li key={language.name} className="pl2 pv2 ba bg-white dib w-50 w-100-l db-l b--dotted b--light-silver">
        <span style={{color: language.color}} className="dib">•</span> {language.name}
      </li>
    );
  });
}
